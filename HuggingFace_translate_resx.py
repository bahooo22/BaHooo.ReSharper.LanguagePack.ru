...
def translate_file(src_path, ru_new_path, log_mode, translator, resume, log_file):
    ...
    for idx, data in enumerate(tqdm(values, desc=f"{os.path.basename(src_path)} zh→ru", unit="строка"), start=1):
        value = data.find("value")
        if value is not None and value.text:
            raw = value.text
            final = cached_translate(translator, raw)
            final = preserve_placeholders(raw, final)
            value.text = final
            if log_mode == "full":
                logs.append(f"[{idx}] zh: {raw} | ru: {final}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Translation of .resx files: zh→ru with caching, placeholder preservation, grouped logging, parallel workers, resume, dry-run.",
        formatter_class=argparse.RawTextHelpFormatter,
        epilog="""
Примеры запуска:

1. Все файлы (минимальный лог):
   python translate_resx.py --original C:\\src --final C:\\ru

2. Один файл для отладки:
   python translate_resx.py --original C:\\src --final C:\\ru --single MyPlugin.Strings.zh.resx

3. Полный лог:
   python translate_resx.py --original C:\\src --final C:\\ru --log full

4. Первые 5 фраз:
   python translate_resx.py --original C:\\src --final C:\\ru --log first5

5. Каждая 10-я фраза:
   python translate_resx.py --original C:\\src --final C:\\ru --log every10

6. Параллельная обработка 4 файлов:
   python translate_resx.py --original C:\\src --final C:\\ru --workers 4

7. Продолжение обработки:
   python translate_resx.py --original C:\\src --final C:\\ru --resume

8. Комбинация: один файл, полный лог, resume:
   python translate_resx.py --original C:\\src --final C:\\ru --single MyPlugin.Strings.zh.resx --log full --resume

9. Все файлы, каждая 10-я фраза, 2 воркера:
   python translate_resx.py --original C:\\src --final C:\\ru --log every10 --workers 2

10. Проверка списка файлов без перевода:
   python translate_resx.py --original C:\\src --final C:\\ru --dry-run

11. Сохранение лога в файл:
   python translate_resx.py --original C:\\src --final C:\\ru --log full --workers 2 --resume --logfile C:\\logs\\translate.log
"""
    )
    ...
    # Инициализация HuggingFace NLLB‑200
    translator = pipeline(
        "translation",
        model="facebook/nllb-200-distilled-600M",
        src_lang="zh_CN",
        tgt_lang="ru",
        device=-1,  # CPU, можно поставить 0 для GPU
        model_kwargs={"num_beams": 5, "max_length": 300}
    )
    ...
    def task(file):
        ...
        ru_new_path = os.path.join(args.final, file.replace(".resx", ".ru-RU.resx.new"))
        ...
        print(msg)
        if log_file: log_file.write(msg + "\n")
        translate_file(src_path, ru_new_path, args.log, translator, args.resume, log_file)
